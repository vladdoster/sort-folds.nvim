...
if vim.g.loaded_sort_folds == 1 then
  return
end
vim.g.loaded_sort_folds = 1

-- Set default configuration if not already set
if vim.g.sort_folds_ignore_case == nil then
...
vim.keymap.set('v', '<Plug>SortFolds', function()
  vim.cmd("'<,'>SortFolds")
end, { silent = true, desc = 'Sort folds in visual selection' })
